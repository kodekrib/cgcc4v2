...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.mountainOfInfluence.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.mountain-of-influences.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('my_mountain_of_culture') ? 'is-invalid' : '' }}" name="my_mountain_of_culture_id" id="my_mountain_of_culture_id">
                    @foreach($my_mountain_of_cultures as $id => $entry)
                        <option value="{{ $id }}" {{ old('my_mountain_of_culture_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
