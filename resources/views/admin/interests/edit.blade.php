...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.interest.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.interests.update", [$interest->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('interests') ? 'is-invalid' : '' }}" name="interests[]" id="interests" multiple>
                    @foreach($interests as $id => $interest)
                        <option value="{{ $id }}" {{ (in_array($id, old('interests', [])) || $interest->interests->contains($id)) ? 'selected' : '' }}>{{ $interest }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('other_sports') ? 'is-invalid' : '' }}" type="text" name="other_sports" id="other_sports" value="{{ old('other_sports', $interest->other_sports) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('social_causes') ? 'is-invalid' : '' }}" type="text" name="social_causes" id="social_causes" value="{{ old('social_causes', $interest->social_causes) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('entrepreneurial_interests') ? 'is-invalid' : '' }}" type="text" name="entrepreneurial_interests" id="entrepreneurial_interests" value="{{ old('entrepreneurial_interests', $interest->entrepreneurial_interests) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('industry_sector') ? 'is-invalid' : '' }}" name="industry_sector_id" id="industry_sector_id">
                    @foreach($industry_sectors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('industry_sector_id') ? old('industry_sector_id') : $interest->industry_sector->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
