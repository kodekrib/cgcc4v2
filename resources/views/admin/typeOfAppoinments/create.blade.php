...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.typeOfAppoinment.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.type-of-appoinments.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('members_list') ? 'is-invalid' : '' }}" name="members_list[]" id="members_list" multiple required onchange="onManage()">
                    @foreach($memberManageAppointmentType as $id => $members_item)
                        <option value="{{ $id }}" {{ in_array($id, old('members_list', [])) ? 'selected' : '' }}>{{ $members_item }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('default_members_id') ? 'is-invalid' : '' }}" name="default_members_id" id="default_members_id">
                    ...
                    @foreach($memberManageAppointmentType as $key => $label)
                        @if (collect(old('members_list'))->contains($key))
                            <option value="{{ $key }}" {{ old('default_members_id', '') ===  $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endif
                    @endforeach
                </select>
