...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.employmentDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.employment-details.update", [$employmentDetail->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="employer_name">{{ trans('cruds.employmentDetail.fields.employer_name') }}</label>
                <input class="form-control {{ $errors->has('employer_name') ? 'is-invalid' : '' }}" type="text" name="employer_name" id="employer_name" value="{{ old('employer_name', $employmentDetail->employer_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('employer_address') ? 'is-invalid' : '' }}" type="text" name="employer_address" id="employer_address" value="{{ old('employer_address', $employmentDetail->employer_address) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('employer_address_2') ? 'is-invalid' : '' }}" type="text" name="employer_address_2" id="employer_address_2" value="{{ old('employer_address_2', $employmentDetail->employer_address_2) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('country') ? 'is-invalid' : '' }}" name="country" id="country"  onchange="changeCountry()">
                    ...
                    @foreach($countries as $country => $data)
                        <option value="{{ $data['name']  }}" {{ old('country', $employmentDetail->country) === (string)$data['name'] ? 'selected' : '' }}>{{ $data['name']  }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group"  id="state_dropDown">
                ...
                <select class="form-control {{ $errors->has('state') ? 'is-invalid' : '' }}" name="state" id="state">
                    ...
                    @foreach($states as $state => $label)
                        <option value="{{ $label['state'] }}" {{ old('state', $employmentDetail->state) === (string) $label['state'] ? 'selected' : '' }}>{{ $label['state'] }}</option>
                    @endforeach
                </select>
                @if($errors->has('state'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group" id="state_text">
                <label>{{ trans('cruds.employmentDetail.fields.state') }}</label>
                <input class="form-control {{ $errors->has('state') ? 'is-invalid' : '' }}" type="text" name="state" id="state_str"  value="{{ old('state', $employmentDetail->state) }}">
                @if($errors->has('state'))
                    <div class="invalid-feedback">
                        {{ $errors->first('state') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('city') ? 'is-invalid' : '' }}" type="text" name="city" id="city" value="{{ old('city', $employmentDetail->city) }}">
                @if($errors->has('city'))
                    <div class="invalid-feedback">
                        {{ $errors->first('city') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('position_held') ? 'is-invalid' : '' }}" type="text" name="position_held" id="position_held" value="{{ old('position_held', $employmentDetail->position_held) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('industry') ? 'is-invalid' : '' }}" name="industry_id" id="industry_id">
                    @foreach($industries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('industry_id') ? old('industry_id') : $employmentDetail->industry->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('subsector') ? 'is-invalid' : '' }}" name="subsector_id" id="subsector_id">
                    @foreach($subsectors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('subsector_id') ? old('subsector_id') : $employmentDetail->subsector->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
