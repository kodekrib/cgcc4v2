...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-memberAffinityGroupMembersAffinityGroups">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <th>
                            {{ trans('cruds.membersAffinityGroup.fields.member_affinity_group') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                ...
                <tbody>
                    @foreach($membersAffinityGroups as $key => $membersAffinityGroup)
                        <tr data-entry-id="{{ $membersAffinityGroup->id }}">
                            ...
                            <td>
                                @foreach($membersAffinityGroup->member_affinity_groups as $key => $item)
                                    <span class="badge badge-info">{{ $item->affinity_group }}</span>
                                @endforeach
                            </td>
                            <td>
...
@endcan
  ...
  let table = $('.datatable-memberAffinityGroupMembersAffinityGroups:not(.ajaxTable)').DataTable({ buttons: dtButtons })
