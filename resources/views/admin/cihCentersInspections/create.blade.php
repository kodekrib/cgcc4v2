...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.cihCentersInspection.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.cih-centers-inspections.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="date_of_inspection">{{ trans('cruds.cihCentersInspection.fields.date_of_inspection') }}</label>
                <input class="form-control date {{ $errors->has('date_of_inspection') ? 'is-invalid' : '' }}" type="text" name="date_of_inspection" id="date_of_inspection" value="{{ old('date_of_inspection') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('center_visited') ? 'is-invalid' : '' }}" name="center_visited_id" id="center_visited_id">
                    @foreach($center_visiteds as $id => $entry)
                        <option value="{{ $id }}" {{ old('center_visited_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('summary_of_visit') ? 'is-invalid' : '' }}" name="summary_of_visit" id="summary_of_visit">{!! old('summary_of_visit') !!}</textarea>
