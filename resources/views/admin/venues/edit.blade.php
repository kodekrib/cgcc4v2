    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.venues.update", [$venue->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('venue_description') ? 'is-invalid' : '' }}" name="venue_description" id="venue_description">{{ old('venue_description', $venue->venue_description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('accessories_equipments') ? 'is-invalid' : '' }}" name="accessories_equipments[]" id="accessories_equipments" multiple required>
                    @foreach($accessories_equipments as $id => $accessories_equipment)
                        <option value="{{ $id }}" {{ (in_array($id, old('accessories_equipments', [])) || $venue->accessories_equipments->contains($id)) ? 'selected' : '' }}>{{ $accessories_equipment }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('accessibility_features') ? 'is-invalid' : '' }}" name="accessibility_features[]" id="accessibility_features" multiple>
                    @foreach($accessibility_features as $id => $accessibility_feature)
                        <option value="{{ $id }}" {{ (in_array($id, old('accessibility_features', [])) || $venue->accessibility_features->contains($id)) ? 'selected' : '' }}>{{ $accessibility_feature }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('venue_capacity') ? 'is-invalid' : '' }}" type="number" name="venue_capacity" id="venue_capacity" value="{{ old('venue_capacity', $venue->venue_capacity) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('size') ? 'is-invalid' : '' }}" type="number" name="size" id="size" value="{{ old('size', $venue->size) }}" step="1">
                @if($errors->has('size'))
                    <div class="invalid-feedback">
                        {{ $errors->first('size') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('venue_location') ? 'is-invalid' : '' }}" name="venue_location_id" id="venue_location_id" required>
                    @foreach($venue_locations as $id => $entry)
                        <option value="{{ $id }}" {{ (old('venue_location_id') ? old('venue_location_id') : $venue->venue_location->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\Venue::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $venue->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
