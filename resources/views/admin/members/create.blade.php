    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.members.store") }}" enctype="multipart/form-data">
            ...
            {{-- <div class="form-group">
                <div class="form-check {{ $errors->has('born_in_nigeria') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="born_in_nigeria" value="0">
                    <input class="form-check-input" type="checkbox" name="born_in_nigeria" id="born_in_nigeria" value="1" {{ old('born_in_nigeria', 0) == 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="born_in_nigeria">{{ trans('cruds.member.fields.born_in_nigeria') }}</label>
                </div>
                @if($errors->has('born_in_nigeria'))
                    <div class="invalid-feedback">
            ...
            {{-- <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('country_of_birth') ? 'is-invalid' : '' }}" name="country_of_birth" id="country_of_birth">
                    <option value disabled {{ old('country_of_birth', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Member::COUNTRY_OF_BIRTH_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('country_of_birth', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('country_of_birth'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group" id="state_of_origin_text">
                ...
                <input class="form-control {{ $errors->has('state_of_origin') ? 'is-invalid' : '' }}" type="text" name="state_of_origin"  value="{{ old('state_of_origin', '') }}">
