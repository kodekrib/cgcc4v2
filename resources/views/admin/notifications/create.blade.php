    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.notifications.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('message_title') ? 'is-invalid' : '' }}" type="text" name="message_title" id="message_title" value="{{ old('message_title', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('phone_number') ? 'is-invalid' : '' }}" name="phone_number_id" id="phone_number_id">
                    @foreach($phone_numbers as $id => $entry)
                        <option value="{{ $id }}" {{ old('phone_number_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('emails') ? 'is-invalid' : '' }}" name="emails[]" id="emails" multiple>
                    @foreach($emails as $id => $email)
                        <option value="{{ $id }}" {{ in_array($id, old('emails', [])) ? 'selected' : '' }}>{{ $email }}</option>
                    @endforeach
                </select>
