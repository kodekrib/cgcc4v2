...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.empowerment.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.empowerments.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ats_membership_no') ? 'is-invalid' : '' }}" name="ats_membership_no_id" id="ats_membership_no_id">
                    @foreach($ats_membership_nos as $id => $entry)
                        <option value="{{ $id }}" {{ old('ats_membership_no_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('cooperative') ? 'is-invalid' : '' }}" name="cooperative" id="cooperative" required onchange="onCooperativeSelected()">
                    ...
                    @foreach(App\Models\Empowerment::COOPERATIVE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('cooperative', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div id="container_coperative">
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('contribution_amount') ? 'is-invalid' : '' }}" type="number" name="contribution_amount" id="contribution_amount" value="{{ old('contribution_amount', '0.00') }}" step="0.01">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('contribution_frequency') ? 'is-invalid' : '' }}" name="contribution_frequency" id="contribution_frequency" >
                        ...
                        @foreach(App\Models\Empowerment::CONTRIBUTION_FREQUENCY_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('contribution_frequency', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('start_year') ? 'is-invalid' : '' }}" type="number" name="start_year" id="start_year" value="{{ old('start_year', '2023') }}" step="1" >
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('start_month') ? 'is-invalid' : '' }}" name="start_month" id="start_month" >
                        ...
                        @foreach(App\Models\Empowerment::START_MONTH_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('start_month', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('business_advisory') ? 'is-invalid' : '' }}" name="business_advisory" id="business_advisory" required onchange="onAdvisoryTeamSelected()">
                    ...
                    @foreach(App\Models\Empowerment::BUSINESS_ADVISORY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('business_advisory', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group" id="container_advisory_team">
                ...
                <input class="form-control {{ $errors->has('advisory_team') ? 'is-invalid' : '' }}" type="text" name="advisory_team" id="advisory_team" value="{{ old('advisory_team', '') }}" >
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('trainings') ? 'is-invalid' : '' }}" name="trainings" id="trainings" onchange="onTrainingNeedsSelected()">
                    ...
                    @foreach(App\Models\Empowerment::TRAININGS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('trainings', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group" id="container_training_needs">
                ...
                <select class="form-control select2 {{ $errors->has('training_needs') ? 'is-invalid' : '' }}" name="training_needs[]" id="training_needs" multiple>
                    @foreach($training_needs as $id => $training_need)
                        <option value="{{ $id }}" {{ in_array($id, old('training_needs', [])) ? 'selected' : '' }}>{{ $training_need }}</option>
                    @endforeach
                </select>
